            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="mt-1 text-capitalize">{{ request()->segment(1, 1) }} Presensi Bulan <strong
                            class="text-danger">{{ $bulan[$selected] }}</strong> Tahun {{ $tahun ?? '' }} <span
                            class="float-right">
                            <small>
                    ...
                    <div class="row mb-3">
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination">
                                        ...
                                        <li class="page-item {{ $selected == $key ? 'active':'' }}"><a class="page-link"
                                                href="{{ $val == 'libur' ? '#':'?bulan='.$key.'&jadwal='.$jdw.'&tahun='.$tahun.'&cari='.$cari }}">{!!
                                                $val !!}</a></li>
                                        @endforeach
                                        <li class="page-item"><a class="page-link bg-warning text-dark"
                                                href="{{ route('rekap.index') }}">Reset</a></li>
                        ...
                        <div class="col-md-12 col-lg-1 col-sm-12">
                            <select class="form-control" id="pilih_tahun">
                                ...
                                @foreach (range(now()->year-3, now()->year) as $item)
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center align-middle" style="font-size: 10pt"
                                        colspan="{{ count($hari) }}">Bulan {{$bulan[$selected].' '.$tahun }}</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="align-baseline text-center" rowspan="2" style="font-size: 10pt">
                                        ...
                                        <a target="t_blank" href="{{ route('rekap.show', $item->username) }}?action=detail&bulan={{ $selected }}&jadwal={{ $jdw }}&tahun={{ $tahun }}"
                                            class="btn btn-xs btn-primary mr-1 ml-1 mb-1">Detail</a>
                                        <a target="t_blank" href="{{ route('rekap.show', $item->username) }}?action=excel&bulan={{ $selected }}&jadwal={{ $jdw }}&tahun={{ $tahun }}"
                                    ...
                                    1)->whereStatusPresensiId(1)->whereAutoPresensi(0)->whereMonth('waktu', $selected)->whereYear('waktu', $tahun)->get();
                                    ...
                                    3)->whereStatusPresensiId(1)->whereAutoPresensi(0)->whereMonth('waktu', $selected)->whereYear('waktu', $tahun)->get();
                                    ...
                                    1)->whereStatusPresensiId(1)->whereMonth('waktu', $selected)->whereYear('waktu', $tahun)->whereDate('waktu',
                                    \Carbon\CarbonImmutable::create($tahun, $selected, $key+1))->first();
                                    $checked = \Carbon\Carbon::parse($dapat->waktu)->format('H:i');
                                    }else{
                                        //test
                                    $getStatus = $item->presensi()->where('user_id', $item->id)->where('tipe',
                                    1)->whereMonth('waktu', $selected)->whereYear('waktu', $tahun)->whereDate('waktu',
                                    \Carbon\CarbonImmutable::create($tahun, $selected, $key+1))->first();
                                    $checked = isset($getStatus['statusPresensi']['kode']);
                                    ...
                                    <td class="text-center table-info font-weight-bold" style="font-size: 12px">
                                        ...
                                        {{ \App\Jadwal::getLate($item->id, $selected, $tahun)->telat_totalv2 }}
                                    </td>
                                    <td style="font-size: 10pt" class="text-center table-info font-weight-bold">
                                ...
                                <tr class="">
                                    ...
                                    $dapat = $item->presensi()->where('user_id', $item->id)->where('tipe',
                                    3)->whereStatusPresensiId(1)->whereMonth('waktu', $selected)->whereYear('waktu', $tahun)->whereDate('waktu',
                                    \Carbon\CarbonImmutable::create($tahun, $selected, $key+1))->first();
                                    $checked = \Carbon\Carbon::parse($dapat->waktu)->format('H:i');
                                    }else{
                                    // $checked = '<i class="align-middle bx bx-x text-danger"></i>';
...
<script>
    function cetakfull(){
            ...
            target.src = "{{ route('rekap.cetak', 'full') }}?bulan={{ $selected }}&jadwal={{ $jdw }}&tahun={{ $tahun ?? date('Y') }}";
        }
        ...
        if (this.value !== '') {
        let url = "?bulan={{ $selected ?? '' }}&jadwal={{ $jdw ?? '' }}&tahun="+this.value+"&cari={{ $cari ?? '' }}"
        window.location.href = url;
        }
        });
</script>
